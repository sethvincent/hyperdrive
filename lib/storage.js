...
function ChunkStore (feed) {
  if (!(this instanceof ChunkStore)) return new ChunkStore(feed)
  ...
  this.db = null
  this.filename = null
  this.contentBlocks = 0 //link.blocks - link.index.length
  ...
  this.open = thunky(function (cb) {
    upget(function (err, doc) {
      ...
      self.db = feed.cache.db

      var acc = 0
      for (var i = 0; i < doc.index.length; i++) {
        self._index[i] = acc += doc.index[i]
      }
  ...
  function upget (cb) {
    feed.cache.get('info', {valueEncoding: 'json'}, function (_, doc) {
      if (doc) return cb(null, doc)
      var doc = {
        filename: feed.options.filename,
        index: feed.options.index || []
      }
      feed.cache.put('info', doc, {valueEncoding: 'json'}, function (err) {
        if (err) return cb(err)
        cb(null, doc)
      })
    })
  ...
  function flush (_, cb) {
    ...
    while (self._writeOffsets.length) {
      ...
      batch.push({
        type: 'put',
        key: self._feed.cache.prefix + '!offsets!' + index,
...
ChunkStore.prototype._append = function (bufs, cb) {
  if (!this.filename) {
    ...
    for (var i = 0; i < bufs.length; i++) {
      batch[i] = {
        type: 'put',
        key: this._feed.cache.prefix + '!data!' + (this.contentBlocks++),
        value: bufs[i]
      }
    }

    this._feed.cache.db.batch(batch, cb)
    return
  }
  ...
  for (var i = 0; i < bufs.length; i++) {
    if (!this._nextIndex || this._nextPtr === this._nextIndex.length) {
      ...
      this._nextIndex = new Buffer(16768)
      this._nextPtr = 0
    }
    var buf = bufs[i]
    this.contentBlocks++
...
ChunkStore.prototype.finalize = function (cb) {
  ...
  this._feed.cache.put('info', doc, {valueEncoding: 'json'}, function (err) {
...
ChunkStore.prototype.get = function (index, cb) {
  ...
  if (!this.filename) {
    this._feed.cache.get('!data!' + index, cb)
    return
  }
  if (index >= this.contentBlocks) {
    this._feed.cache.get('!blocks!' + index, cb)
    return
  }
...
ChunkStore.prototype.put = function (index, buf, cb) {
  ...
  if (!this.filename) {
    this._feed.cache.put('!data!' + index, buf, cb)
    return
  }
  ...
  if (index >= this.contentBlocks) {
    ...
    this.db.batch([{
      type: 'put',
      key: this._feed.cache.prefix + '!blocks!' + index,
      value: buf
    }, {
      type: 'put',
      key: this._feed.cache.prefix + '!offsets!' + (index - this.contentBlocks),
      value: inflated
    }], cb)
    return
  }
...
ChunkStore.prototype._load = function (i, cb) {
  ...
  this.open(function (err) {
    if (err) return cb(err)
    self._feed.cache.get('!offsets!' + i, function (_, buf) {
